...
const ohMy = () => {
    ...
    for(let i = 0; i < res.data.length; i++ ){
        let kangaroo = document.createElement('p')
        kangaroo.textContent = res.data[i]
        let joey = document.querySelector('body')
    joey.appendChild(kangaroo)
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/UnMeGonFisin').then((cb) => {
    console.log(cb.data)
    let paramText = document.getElementById('repeat-text')
    paramText.textContent = cb.data;
    paramText.style.display = 'block'
})
...
const queryTester = () => {
    axios.get('http://localhost:3000/query-test?').then((cb) => {
        console.log(cb.data)
})
}
...
/*
    ...
    response was: 'you sent an empty query!'

    2: Send more than 1 query on the URL -- what happened? 

    response was: "you sent more than 1 query"
*/
